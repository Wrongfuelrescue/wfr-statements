...
import FortnightlyPage from '../page'
...
describe('FortnightlyPage no-ABN warning', () => {
  ...
  it('does not block submission — Review is still reachable', async () => {
    vi.mocked(requireContractorId).mockResolvedValue('rec36VBHdVAy4XyuY')
    vi.mocked(getRateCard).mockResolvedValue({ ...rates, abn: '' })

    render(await FortnightlyPage())

    expect(screen.getByRole('button', { name: /review statement/i })).toBeInTheDocument()
  })
})
